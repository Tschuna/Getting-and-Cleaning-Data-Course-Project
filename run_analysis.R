...
## Optional: Remove unneeded data from Global Environment
rm(testData)
rm(trainData)
...
## Read the features list (to be used as column names for the data)
featuresList <- read.table("features.txt", stringsAsFactors=FALSE)

## Use only names from the features list
features <- featuresList$V2

## Create a logical vector to keep only the mean and standard deviation columns
...
## Add clean labels to activity names
names(S) <- "subject"
